...
export function createTargetIdentityKeys(kind: TargetKind, index: number, name: string): string[] {
  const nameKey = normalizeName(name);
  return [`${kind}:name:${nameKey}`, `${kind}:index:${index}`];
}

export function buildTargetId(kind: TargetKind, index: number, name: string): string {
...
export function mergeTargetName(kind: TargetKind, index: number, label?: string): string {
