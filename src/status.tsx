...
import { launchVoicemeeterFromSettings, undoLastChange } from "./lib/controller";
...
function byKind(targets: VoicemeeterTarget[], kind: "strip" | "bus"): VoicemeeterTarget[] {
...
export default function Command() {
  ...
  const strips = filterVisible(sortWithFavoritesFirst(byKind(state.targets, "strip"), favorites), hidden);
  const buses = filterVisible(sortWithFavoritesFirst(byKind(state.targets, "bus"), favorites), hidden);
  const hiddenTargets = state.targets.filter((t) => isHidden(t, hidden));

  return (
    <List isLoading={isLoading} navigationTitle="Status" searchBarPlaceholder="Filter snapshot...">
            ...
            .map((strip) => (
              <List.Item
                ...
                actions={
                  <ActionPanel>
                    <Action
                      title={isFavorite(strip, favorites) ? "Remove from Favorites" : "Add to Favorites"}
                      onAction={() => toggleFavorite(strip)}
                      icon={Icon.Star}
                    />
                    <Action title="Hide" onAction={() => toggleHidden(strip)} icon={Icon.EyeDisabled} />
                    <Action title="Refresh Snapshot" onAction={refreshEverything} />
                    <Action title={`Undo Last Change (${undoCount})`} onAction={handleUndo} />
                    <Action.Push title="Quick Settings" target={<QuickSettingsForm onSaved={refreshEverything} />} icon={Icon.Gear} />
                  </ActionPanel>
